...
data class FirebaseUser (val id: String, var score: Int, val displayName: String? = null, var rating: Int = 0,
                         ...
                         var following: Map<String, Boolean>? = null, var profilePictureMetadata: ProfilePhotoMetadata? = null,
    ...
    constructor(user: User) : this( user.id, user.score, user.displayName, user.rating,
    ...
    fun toUser(): User {
        return User(id, score, displayName, rating, description, phone, email, progression,
        followers?.keys?.toList(), following?.keys?.toList(), profilePictureMetadata, trashPhotosMetadatasList)
    }
}
