...
class BinTest {
    ...
    fun equalsOnBinsOnlyComparesFields() {
        val bin1 = Bin("id", TrashCategory.PAPER, 69.69, 4.20)
        val bin2 = Bin("id", TrashCategory.PAPER, 69.69, 4.20)
        assertEquals(bin1, bin2)
    }

    @Test
    fun defaultConstructorCreatesExpectedDefaultBin() {
        assertEquals(Bin(), Bin(null, TrashCategory.PLASTIC,0.0, 0.0))
    }
    ...
    fun locationReturnsTheRightLocation() {
        val bin = Bin("id1", TrashCategory.ORGANIC, 4.32, 3.25)
